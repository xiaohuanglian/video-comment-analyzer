...
从真实评论中发现需求、痛点与商机的采集与分析工具。提供 Web 界面，支持多平台视频/笔记评论抓取，以及 **评论洞察**（默认证据分析）、**评论区回复**（筛选值得回复的用户 + 生成公开回复草稿）与 **内容选题**（把需求转成选题并生成多形态草稿）。所有需要 AI 的生成环节（回复草稿、内容选题）都<strong>必须人工点击开始</strong>，且<strong>只生成草稿、永不自动发送/发布</strong>。
...
8. **下游步骤**（均需手动触发）：
   - 「评论区回复」筛选值得回复的用户 → 勾选 →「生成回复草稿」→ 编辑后**手动复制发送**
   - 「内容选题」基于评论需求生成内容选题与多形态草稿 → 人工审核后再发布
9. **导出**：分析完成后自动写入 CSV 同目录（`{任务名}_分析结果.csv`、`_洞察报告.md`）；生成回复对象/内容选题后也会自动保存对应文件
