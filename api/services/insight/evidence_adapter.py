...
def has_explicit_paid_action(text: str) -> bool:
    ...
    return any(keyword in value for keyword in PAID_ACTION_KEYWORDS) or bool(
        re.search(r"我.{0,10}(报了|报名|买了|购买|付了|花了|充值|订阅|请了|找了.{0,5}(专业人士|顾问|医生))", value)
    )
...
def infer_legacy_signals(card: EvidenceCard | dict) -> List[str]:
    ...
    behavior_map = {
        ...
        "attempted": "started_using",
        "completed_once": "started_using",
        "continued": "continued_using",
        "ongoing_period": "continued_using",
        "stopped": "stopped_using",
    ...
    gap_map = {
        "saved_but_not_started": "skipped_step",
        "watched_but_not_practiced": "skipped_step",
        "started_but_stopped": "stopped_using",
        "planned_but_not_started": "skipped_step",
    }
    ...
    for item in card.evidence_items or []:
        ...
        if item.type == EvidenceItemType.ENGAGEMENT:
            ...
            elif any(k in text for k in ("没感觉", "无感", "感受不到", "没效果")):
                signals.add("expected_effect_missing")
            elif any(k in text for k in ("有效", "有用", "改善", "学会", "进步", "立竿见影")):
                signals.add("positive_result")
            elif text.strip():
                signals.add("positive_result")
        elif item.type == EvidenceItemType.PROBLEM:
            if any(k in text for k in ("标准", "对不对", "是不是我", "姿势", "做得不", "方法不")):
                signals.add("howto_uncertainty")
            ...
            if any(k in text for k in ("没感觉", "感受不到", "没效果")):
                signals.add("expected_effect_missing")
            if any(k in text for k in ("疼", "痛", "伤", "不适")):
                signals.add("physical_discomfort")
            if any(k in text for k in ("替代", "换成", "没有器械", "没工具", "没有条件")):
                signals.add("needs_alternative")
            if any(k in text for k in ("进阶", "加大", "提高难度")):
                signals.add("needs_advanced")
            if any(k in text for k in ("退阶", "简化", "降低难度")):
                signals.add("needs_simpler")
            if any(k in text for k in ("计划", "安排", "怎么练", "怎么做", "从哪开始")):
                signals.add("needs_plan")
        elif item.type == EvidenceItemType.BARRIER:
            if any(k in text for k in ("疼", "痛", "伤")):
                signals.add("physical_discomfort")
            if any(k in text for k in ("太难", "做不到", "做不了")):
                signals.add("cannot_complete")
            if any(k in text for k in ("空间", "器械", "设备", "工具", "条件")):
                signals.add("resource_constraint")
            if any(k in text for k in ("隐私", "尴尬")):
                signals.add("privacy_concern")

    if expression == PrimaryExpression.QUESTION.value:
        if not signals & {"howto_uncertainty", "applicability_question"}:
            if any(k in pool for k in ("标准", "对不对", "是不是")):
                signals.add("howto_uncertainty")
            else:
                signals.add("applicability_question")
    if any(k in pool for k in ("回复", "看看我", "帮我看看", "博主回", "作者回")):
        signals.add("asks_creator_reply")
    if any(k in pool for k in ("录像", "拍视频", "录自己", "拍下来")):
        signals.add("recorded_self_for_review")
    if any(k in pool for k in ("坚持", "签到", "动力", "懒", "自律")):
    ...
    if any(k in pool for k in ("太快", "跟不上", "节奏", "数")):
        signals.add("pace_problem")
    if any(k in pool for k in ("旧伤", "受伤", "手术", "慢性病", "过敏")):
        signals.add("special_condition")
