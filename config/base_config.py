...
# Web UI mode: set by api.main when running the FastAPI server
WEB_UI_MODE = False
...
CRAWLER_MAX_COMMENTS_COUNT_SINGLENOTES = 999999

# Whether to enable the mode of crawling second-level comments. By default, crawling of second-level comments is not enabled.
# If the old version of the project uses db, you need to refer to schema/tables.sql line 287 to add table fields.
ENABLE_GET_SUB_COMMENTS = False

# Stop a comment pass after this many consecutive pages with no new records (dedupe / empty pages).
BILI_MAX_EMPTY_COMMENT_PAGES = 25
...
# Random extra delay on each wait (seconds), mimics human browsing rhythm
CRAWLER_SLEEP_JITTER_SEC = 1.5

# Adaptive throttle: increase delay after rate-limit errors, decay on success
ENABLE_ADAPTIVE_THROTTLE = True
ADAPTIVE_THROTTLE_MAX_MULTIPLIER = 6

# Extra pause every N comment pages (0 = disabled)
CRAWLER_PAGE_BATCH_PAUSE_EVERY = 10
CRAWLER_PAGE_BATCH_PAUSE_SEC = 8

# Safer defaults: longer intervals, batch pauses, adaptive backoff
ENABLE_SAFE_CRAWL_MODE = True

# Save each video's data under {SAVE_DATA_PATH}/{title_slug}_{video_id}/
ENABLE_VIDEO_FOLDER_SPLIT = True

# Resume comment crawl from existing CSV + checkpoint when re-running the same video
ENABLE_COMMENT_RESUME = False

# Automatically delete incomplete per-video output and restart from scratch
AUTO_RESTART_PARTIAL_CRAWL = True

# Force delete existing per-video output before crawling (CLI / API override)
FRESH_CRAWL = False
