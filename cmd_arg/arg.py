...
async def parse_cmd(argv: Optional[Sequence[str]] = None):
    ...
    def main(
        ...
        ] = os.environ.get("VC_CRAWLER_COOKIES", config.COOKIES),
        ...
        enable_safe_crawl: Annotated[
            str,
            typer.Option(
                "--enable_safe_crawl",
                help="Enable safer crawl pacing (longer jitter, adaptive throttle, batch pauses)",
                rich_help_panel="Performance Configuration",
                show_default=True,
            ),
        ] = str(config.ENABLE_SAFE_CRAWL_MODE),
        split_by_video: Annotated[
            str,
            typer.Option(
                "--split_by_video",
                help="Save each video under its own subfolder named by title slug and video id",
                rich_help_panel="Basic Configuration",
                show_default=True,
            ),
        ] = str(config.ENABLE_VIDEO_FOLDER_SPLIT),
        fresh_crawl: Annotated[
            str,
            typer.Option(
                "--fresh_crawl",
                help="Delete existing per-video output files before crawling",
                rich_help_panel="Basic Configuration",
                show_default=True,
            ),
        ] = str(config.FRESH_CRAWL),
        crawler_max_sleep_sec: Annotated[
            float,
            typer.Option(
                "--crawler_max_sleep_sec",
                help="Base delay between crawl requests (seconds)",
                rich_help_panel="Performance Configuration",
            ),
        ...
        enable_safe_crawl_value = _to_bool(enable_safe_crawl)
        split_by_video_value = _to_bool(split_by_video)
        fresh_crawl_value = _to_bool(fresh_crawl)
        ...
        config.ENABLE_SAFE_CRAWL_MODE = enable_safe_crawl_value
        config.ENABLE_VIDEO_FOLDER_SPLIT = split_by_video_value
        config.FRESH_CRAWL = fresh_crawl_value
        if fresh_crawl_value:
            config.ENABLE_COMMENT_RESUME = False
